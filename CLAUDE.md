...
**Social Fabric Foundation (Sprint 16) COMPLETE** — 85/85 tasks, 6 user stories. Follow system (one-way, 200 max, cursor pagination), connection graph (mutual requests, accept/decline, 30-day cooldown, suggestion algorithm with shared domains ×3 + same city ×2 + mutual reviews ×5), low-stakes discussion spaces (domain + city boards, full 3-layer guardrail pipeline, rate limits 10 threads/50 replies per day), personal network dashboard (aggregated interaction history, Redis 5-min cache), care moments (streak-break detection, milestone celebrations, comeback welcome, cheer/celebrate with 1-token gifts via double-entry accounting), contribution ripple effect (recursive chain traversal problem→solution→mission→evidence), notification system (WebSocket push to humans, aggregation, 90-day retention), social metrics analytics. 1429 tests passing (842 API + 43 frontend + 354 guardrails + 233 shared).
...
- **Social Fabric Foundation** (Sprint 16): Follow system (one-way, 200 max, cursor pagination), connection graph (mutual requests, accept/decline, 30-day cooldown, suggestion algorithm), low-stakes discussion spaces (domain + city boards, full 3-layer guardrail integration, 10 threads/50 replies per day), personal network dashboard (aggregated history, Redis 5-min cache), care moments (streak-break detection + milestone celebrations + comeback welcome + cheer/celebrate with 1-token gifts), contribution ripple effect (recursive chain traversal), notification system (human WebSocket push, aggregation, 90-day retention worker), social metrics analytics (Redis daily counters + admin endpoint)
- 1429 tests (354 guardrails + 233 shared + 842 API + 43 frontend) + E2E pipeline test + k6 load test baseline — **all passing**
...
- TypeScript 5.x (strict mode, zero errors), Node.js 22+ + Hono (API), Drizzle ORM, BullMQ, ioredis, jose (JWT), Next.js 15, React Query, Zustand, Tailwind CSS 4 (016-social-fabric-foundation)
- PostgreSQL 16 (Supabase) + Upstash Redis (cache, rate limits, feature flags) (016-social-fabric-foundation)
...
- **2026-02-16: 016-social-fabric-foundation (Sprint 16 — COMPLETE)**: 85 tasks across 10 phases, 6 user stories. Blueprint Spec 1 "Build the Social Fabric" — addresses Care Bonds (D→C+), Friendship (D+→B-), Social Networks (C→B). DB schema (5 new tables: follows, connections, discussion_threads, discussion_replies, notifications; 4 new enums; 2 enum value additions to transactionTypeEnum + contentTypeEnum), migration 0015_social_fabric. Follow system (one-way, 200 max, self-follow prevention, cursor pagination, follow counts on profiles). Connection graph (mutual request/accept/decline, 30-day cooldown, auto-accept mutual pending, suggestion algorithm: shared domains ×3 + same city ×2 + mutual reviews ×5, Redis 5-min cache). Low-stakes discussion spaces (domain + city boards, full 3-layer guardrail pipeline integration, content pending until approved per constitution, 10 threads/50 replies per day rate limits). Personal network dashboard (aggregated followers/connections/interactions, interaction history per partner, Redis 5-min cache). Care moments (streak-break hourly detection, milestone detection on mission completion + tier promotion, comeback welcome on 7+ day return, cheer/celebrate with optional 1-token gift via double-entry accounting). Contribution ripple effect (recursive chain traversal problem→solution→mission→evidence, depth-limited to 5 levels, aggregate "My Ripple" stats). Notification system (human WebSocket /ws/human, notification aggregation, mark read, unread count Redis cache, 90-day retention worker). Social metrics analytics (Redis daily counters, admin GET /admin/social-metrics). Frontend: NotificationBell in nav, notifications page, discussion boards (domain + city), thread detail, FollowButton on portfolios + leaderboard, ConnectButton on profiles, ConnectionSuggestions dashboard card, YourNetworkCard + ImpactRippleCard dashboard cards, network dashboard, impact chain page, CheerButton + CelebrateButton in notifications. 2 new BullMQ workers (care-moment hourly, notification-retention daily). 175 new tests (9 test files). 1429 total tests (842 API + 43 frontend + 354 guardrails + 233 shared).
