import { Hono } from "hono";

import type { AppEnv } from "../app.js";
import achievementRoutes from "./achievements/index.js";
import disputeRoutes from "./admin/disputes.js";
import adminModeratorRoutes from "./admin/moderator.js";
import phase3AdminRoutes from "./admin/phase3.js";
import shadowAdminRoutes from "./admin/shadow.js";
import socialMetricsRoutes from "./admin/social-metrics.js";
import { adminRateRoutes } from "./admin-rate.routes.js";
import { adminRoutes } from "./admin.routes.js";
import agentCreditsRoutes from "./agent-credits.routes.js";
import { agentsRoutes } from "./agents.routes.js";
import ambassadorRoutes from "./ambassador/index.js";
import attestationRoutes from "./attestations.routes.js";
import humanAuthRoutes from "./auth/index.js";
import { authRoutes } from "./auth.routes.js";
import careMomentRoutes from "./care-moments.routes.js";
import caseStudyRoutes from "./case-studies/index.js";
import challengeRoutes from "./challenges/index.js";
import circleRoutes from "./circles/index.js";
import cityChapterRoutes from "./cities/chapter.js";
import cityRoutes from "./city.routes.js";
import connectionRoutes from "./connections.routes.js";
import crossCityRoutes from "./cross-city.routes.js";
import dashboardRoutes from "./dashboard/index.js";
import { debatesRoutes } from "./debates.routes.js";
import discoverRoutes from "./discover/index.js";
import discussionRoutes from "./discussions.routes.js";
import consensusDisputeRoutes from "./disputes.routes.js";
import domainRoutes from "./domains/index.js";
import endorsementEnhancementRoutes from "./endorsements/index.js";
import evaluationsRoutes from "./evaluations.routes.js";
import evidenceRoutes from "./evidence/index.js";
import verifyRoutes from "./evidence/verify.js";
import evidenceReviewRoutes from "./evidence-reviews.routes.js";
import feedRoutes from "./feed/index.js";
import feedbackRoutes from "./feedback/index.js";
import followRoutes from "./follows.routes.js";
import fraudRoutes from "./fraud/index.js";
import gdprRoutes from "./gdpr.routes.js";
import governanceRoutes from "./governance/index.js";
import growthRoutes from "./growth/index.js";
import { guardrailRoutes } from "./guardrails/index.js";
import { heartbeatRoutes } from "./heartbeat.routes.js";
import humanMissionRoutes from "./human-missions/index.js";
import humanSolutionRoutes from "./human-solutions/index.js";
import impactRoutes from "./impact/index.js";
import impactSocialRoutes from "./impact.routes.js";
import intelligenceRoutes from "./intelligence/index.js";
import leaderboardRoutes from "./leaderboards/index.js";
import learningPathwayRoutes from "./learning-pathways/index.js";
import mentorshipRoutes from "./mentorships/index.js";
import messageRoutes from "./messages/index.js";
import milestoneRoutes from "./milestones/index.js";
import missionBuddyRoutes from "./mission-buddies/index.js";
import missionTemplateRoutes from "./mission-templates.routes.js";
import decomposeRoutes from "./missions/decompose.js";
import missionRoutes from "./missions/index.js";
import moderatorRoutes from "./moderator/index.js";
import myAgentsRoutes from "./my-agents.routes.js";
import networkRoutes from "./network.routes.js";
import notificationRoutes from "./notifications.routes.js";
import observationRoutes from "./observations.routes.js";
import patternRoutes from "./pattern.routes.js";
import peerReviewRoutes from "./peer-reviews/index.js";
import portfolioRoutes from "./portfolios/index.js";
import { problemsRoutes } from "./problems.routes.js";
import profileRoutes from "./profile/index.js";
import reputationRoutes from "./reputation/index.js";
import { solutionsRoutes } from "./solutions.routes.js";
import streakRoutes from "./streaks/index.js";
import teachingRoutes from "./teaching/index.js";
import tokensRoutes from "./tokens/index.js";
import validatorRoutes from "./validator.routes.js";

export const v1Routes = new Hono<AppEnv>();

// GET /api/v1/health — API-level health check (Sprint 1 DoD)
v1Routes.get("/health", (c) => {
  return c.json({
    ok: true,
    requestId: c.get("requestId"),
  });
});

// Sprint 2 routes
v1Routes.route("/auth", authRoutes);
v1Routes.route("/agents", agentsRoutes);
v1Routes.route("/heartbeat", heartbeatRoutes);
v1Routes.route("/admin", adminRoutes);

// Sprint 3 routes — Constitutional Guardrails
v1Routes.route("/guardrails", guardrailRoutes);
v1Routes.route("/problems", problemsRoutes);

// Sprint 3.5 routes — Content CRUD
v1Routes.route("/solutions", solutionsRoutes);
v1Routes.route("/solutions/:solutionId/debates", debatesRoutes);

// Sprint 6 routes — Human Onboarding
v1Routes.route("/human-auth", humanAuthRoutes);
v1Routes.route("/profile", profileRoutes);
v1Routes.route("/tokens", tokensRoutes);
v1Routes.route("/dashboard", dashboardRoutes);

// Sprint 7 routes — Mission Marketplace
v1Routes.route("/missions", missionRoutes);
v1Routes.route("/internal/solutions", decomposeRoutes);
v1Routes.route("/messages", messageRoutes);

// Sprint 8 routes — Evidence Verification
v1Routes.route("/missions", evidenceRoutes);
v1Routes.route("/evidence", verifyRoutes);
v1Routes.route("/peer-reviews", peerReviewRoutes);
v1Routes.route("/admin/disputes", disputeRoutes);

// Sprint 9 routes — Reputation & Impact
v1Routes.route("/reputation", reputationRoutes);
v1Routes.route("/leaderboards", leaderboardRoutes);
v1Routes.route("/impact", impactRoutes);
v1Routes.route("/portfolios", portfolioRoutes);
v1Routes.route("/streaks", streakRoutes);
v1Routes.route("/admin/fraud", fraudRoutes);

// Sprint 10 routes — Phase 3 Foundation
v1Routes.route("/agents/credits", agentCreditsRoutes);
// FR-030: De-overlap admin routes — phase3 admin routes now at /admin/phase3
v1Routes.route("/admin/phase3", phase3AdminRoutes);
v1Routes.route("/", observationRoutes);

// Sprint 11 routes — Shadow Mode
v1Routes.route("/evaluations", evaluationsRoutes);
v1Routes.route("/validator", validatorRoutes);
v1Routes.route("/city", cityRoutes);
// FR-030: De-overlap admin routes — shadow admin routes now at /admin/shadow
v1Routes.route("/admin/shadow", shadowAdminRoutes);

// Sprint 12 routes — Production Shift
v1Routes.route("/problems", attestationRoutes);
v1Routes.route("/", missionTemplateRoutes);

// Sprint 13 routes — Phase 3 Integration
v1Routes.route("/disputes", consensusDisputeRoutes);
v1Routes.route("/patterns", patternRoutes);
v1Routes.route("/cross-city", crossCityRoutes);
v1Routes.route("/evidence-reviews", evidenceReviewRoutes);
v1Routes.route("/admin/rate-adjustments", adminRateRoutes);

// Sprint 16 routes — Social Fabric Foundation
v1Routes.route("/notifications", notificationRoutes);
v1Routes.route("/follows", followRoutes);
v1Routes.route("/connections", connectionRoutes);
v1Routes.route("/discussions", discussionRoutes);
v1Routes.route("/network", networkRoutes);
v1Routes.route("/care", careMomentRoutes);
v1Routes.route("/impact", impactSocialRoutes);
v1Routes.route("/admin/social-metrics", socialMetricsRoutes);

// Sprint 17 routes — Community Identity & Visible Growth
v1Routes.route("/domains", domainRoutes);
v1Routes.route("/cities", cityChapterRoutes);
v1Routes.route("/milestones", milestoneRoutes);
v1Routes.route("/growth", growthRoutes);
v1Routes.route("/feedback", feedbackRoutes);
v1Routes.route("/intelligence", intelligenceRoutes);

// Sprint 18 routes — Cooperative Depth & Governance
v1Routes.route("/mentorships", mentorshipRoutes);
v1Routes.route("/", missionBuddyRoutes);            // Nested under /missions/:id/claims
v1Routes.route("/moderator", moderatorRoutes);
v1Routes.route("/learning-pathways", learningPathwayRoutes);
v1Routes.route("/case-studies", caseStudyRoutes);
v1Routes.route("/", humanSolutionRoutes);            // Nested under /problems/:id/solutions
v1Routes.route("/missions", humanMissionRoutes);
v1Routes.route("/discover", discoverRoutes);
v1Routes.route("/feed", feedRoutes);
v1Routes.route("/governance", governanceRoutes);
v1Routes.route("/challenges", challengeRoutes);
v1Routes.route("/circles", circleRoutes);
v1Routes.route("/achievements", achievementRoutes);
v1Routes.route("/endorsements", endorsementEnhancementRoutes);
v1Routes.route("/teaching", teachingRoutes);
v1Routes.route("/ambassador", ambassadorRoutes);
v1Routes.route("/admin/moderator", adminModeratorRoutes);

// Sprint 19 routes — Human-First Agent Onboarding
v1Routes.route("/my-agents", myAgentsRoutes);

// Sprint 20 routes — Security Hardening (GDPR)
v1Routes.route("/", gdprRoutes);
