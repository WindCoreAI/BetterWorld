...
// Broad response type for test assertions (non-null data for direct property access in tests)
interface TestApiResponse {
  ok: boolean;
  data: { id: string; status: string; coolingOffExpiresAt?: string; cancelledAt?: string | null; [key: string]: unknown };
...
describe("GDPR Account Deletion (T024)", () => {
  ...
  describe("GET /me/deletion-request", () => {
    it("should return current deletion request status", async () => {
      vi.mocked(getDeletionStatus).mockResolvedValue({
        id: "request-1",
        status: "pending",
    ...
    it("should return null when no deletion request exists", async () => {
      vi.mocked(getDeletionStatus).mockResolvedValue(null);
