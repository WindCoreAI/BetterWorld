...
**Mitigation**:
- All missions include safety guidelines generated by guardrails
- Missions flagged as potentially hazardous require explicit safety acknowledgment
- Platform disclaimer: BetterWorld is not liable for physical harm; missions are voluntary
- Emergency contact information required during mission claim
- Community reporting mechanism for unsafe missions
- Insurance requirement research for Phase 3 partner missions

### 2.7 Category: Operational (OPS)

| ID | Category | Risk | Severity | Likelihood | Score | Owner | Mitigation Strategy | Contingency Plan | Status |
|----|----------|------|----------|------------|-------|-------|---------------------|------------------|--------|
| OPS-04 | Operational | **Key-person dependency** -- Critical knowledge concentrated in 1-2 team members; departure would halt development | 3 | 4 | **12** | Engineering Lead | Documentation-first culture (all decisions documented in docs/). Pair programming on critical subsystems (guardrails, trust model). Cross-training rotations every 2 sprints. Bus factor target: >= 2 people familiar with each subsystem by end of Phase 1. Comprehensive onboarding guide maintained in CONTRIBUTING.md. | If key person departs: immediate knowledge transfer sessions with remaining team. Prioritize hiring replacement with overlap period. Reduce scope to subsystems with adequate coverage until team is restaffed. | Mitigate |

**Mitigation**:
- Documentation-first culture (all decisions documented in docs/)
- Pair programming on critical subsystems (guardrails, trust model)
- Cross-training rotations every 2 sprints
- Bus factor target: >=2 people familiar with each subsystem by end of Phase 1
- Comprehensive onboarding guide maintained in CONTRIBUTING.md
